...
export class XProjectFactory extends Entity {
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save XProjectFactory entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type XProjectFactory must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("XProjectFactory", id.toString(), this);
    }
  }

  static loadInBlock(id: string): XProjectFactory | null {
    return changetype<XProjectFactory | null>(
      store.get_in_block("XProjectFactory", id)
    );
  }

  static load(id: string): XProjectFactory | null {
    return changetype<XProjectFactory | null>(store.get("XProjectFactory", id));
  }
...
export class XProjectDayData extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save XProjectDayData entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type XProjectDayData must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("XProjectDayData", id.toString(), this);
    }
  }

  static loadInBlock(id: string): XProjectDayData | null {
    return changetype<XProjectDayData | null>(
      store.get_in_block("XProjectDayData", id)
    );
  }

  static load(id: string): XProjectDayData | null {
    return changetype<XProjectDayData | null>(store.get("XProjectDayData", id));
  }
