...
export function handleSync(event: Sync): void {
  ...
  if (!token1) return;
  let xProject = XProjectFactory.load(FACTORY_ADDRESS);
  if (!xProject) return;

  // reset factory liquidity by subtracting onluy tarcked liquidity
  xProject.totalLiquidityETH = xProject.totalLiquidityETH.minus(
  ...
  xProject.totalLiquidityETH = xProject.totalLiquidityETH.plus(
    trackedLiquidityETH
  );
  xProject.totalLiquidityUSD = xProject.totalLiquidityETH.times(
    bundle.ethPrice
  );
...
export function handleMint(event: Mint): void {
  ...
  let token0 = Token.load(pair.token0);
  if (!token0 || !token0.derivedETH) return;
  let token1 = Token.load(pair.token1);
  if (!token1 || !token1.derivedETH) return;
  ...
  xProject.txCount = xProject.txCount.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let derivedAmountETH = derivedToken1AmountETH
    .plus(derivedToken0AmountETH)
  ...
  let xProject = XProjectFactory.load(FACTORY_ADDRESS);
  if (!xProject) return;
  xProject.totalVolumeUSD = xProject.totalVolumeUSD.plus(trackedAmountUSD);
  xProject.totalVolumeETH = xProject.totalVolumeETH.plus(trackedAmountETH);
  xProject.untrackedVolumeUSD = xProject.untrackedVolumeUSD.plus(
    derivedAmountUSD
  );
  xProject.txCount = xProject.txCount.plus(ONE_BI);
  ...
  swap.amountFeeUSD =
    trackedFeeAmountUSD === ZERO_BD ? derivedFeeAmountUSD : trackedFeeAmountUSD;
  swap.save();
  ...
  let xProjectDayData = updateXProjectDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  xProjectDayData.dailyVolumeUSD = xProjectDayData.dailyVolumeUSD.plus(
    trackedAmountUSD
  );
  xProjectDayData.dailyVolumeETH = xProjectDayData.dailyVolumeETH.plus(
    trackedAmountETH
  );
  xProjectDayData.dailyVolumeUntracked = xProjectDayData.dailyVolumeUntracked.plus(
    derivedAmountUSD
  );
  xProjectDayData.save();
